    ...
    def fit(
        ...
        **fit_params,
        ...
        X : array-like of shape (n_observations, n_assets)
           OHLC price & Volume of the assets.
        ...
        daily_return = X.xs("Close", axis=1, level=1).pct_change(1).dropna()
        self._check_feature_names(daily_return, reset=True)
        # Validate
        self._validation()
        ...
        if isinstance(self.prior_estimator_, XGBPrediction):
            self.prior_estimator_.fit(
                X, y, asset_group=asset_group, model_path=model_path
            )
        else:
            self.prior_estimator_.fit(
                daily_return, y, **routed_params.prior_estimator.fit
            )
        ...
        if self.mu_uncertainty_set_estimator is None:
            ...
            self.mu_uncertainty_set_estimator_.fit(
                daily_return, y, **routed_params.mu_uncertainty_set_estimator.fit
            )
        ...
        if self.max_tracking_error is not None:
            ...
            if isinstance(y, pd.DataFrame):
                ...
                y = y[y.columns[0]]
            _, y = self._validate_data(daily_return, y)
        ...
        if self.efficient_frontier_size is not None:
            ...
            model.fit(daily_return, y, **fit_params)
